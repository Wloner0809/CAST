...
case "${DATASET}" in
  id)
    ...
    TRAIN_FILE="${TRAIN_FILE:-data/datasets/minesweeper/id/train_verl.parquet}"
    VAL_FILE="${VAL_FILE:-data/datasets/minesweeper/id/test_verl.parquet}"
    ;;
  unseen)
    DATASET_DIFFICULTY="${DATASET_DIFFICULTY:-unseen-oracle_adv}"
    MAX_STEPS="${MAX_STEPS:-40}"
    TRAIN_FILE="${TRAIN_FILE:-data/datasets/minesweeper/unseen/train_verl.parquet}"
    VAL_FILE="${VAL_FILE:-data/datasets/minesweeper/unseen/test_verl.parquet}"
    ;;
  *)
...
# Minesweeper env computes oracle advantage ONLY when compute_oracle_advantage=True
# (unlike Sokoban whose container always emits it). Requires deterministic boards
# (preset mine_positions + first_click), which the id/unseen
# datasets satisfy.
COMPUTE_ORACLE_ADVANTAGE="${COMPUTE_ORACLE_ADVANTAGE:-True}"
ORACLE_SOLVER_MODE="${ORACLE_SOLVER_MODE:-strong}"  # strong / approximate / weak
...
# advantage range is needed (death = -K can be large; flood/beat-solver steps
# can exceed +1). "off" falls back to plain GRPO.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-${MODEL_BASENAME}-minesweeper-${DATASET_DIFFICULTY}}"
export MODEL_NAME="${MODEL_BASENAME}_BS${TRAIN_BATCH_SIZE}_MPL${MAX_PROMPT_LENGTH}_MRL${MAX_RESPONSE_LENGTH}_MS${MAX_STEPS}-${ORACLE_ADV_MODE}"

agentrl::setup_log_dirs
agentrl::maybe_enable_xtrace

export TIMESTAMP="${TIMESTAMP:-$(date +%Y%m%d%H%M%S)}"
export WANDB_PROJECT="${WANDB_PROJECT:-rLLM-Minesweeper-RL}"
export WANDB_API_KEY="${WANDB_API_KEY:-}"
export WANDB_MODE="${WANDB_MODE:-online}"
export WANDB_DIR="${WANDB_DIR:-${REPO_ROOT:-.}/outputs/minesweeper-}"
...
CKPT_DIR_BASENAME="${CKPT_DIR_BASENAME:-${MODEL_BASENAME}-minesweeper-${DATASET_DIFFICULTY}}"
...
# --- RLLM / Env / Agent ---
ENV_NAME="${ENV_NAME:-minesweeper}"
AGENT_NAME="${AGENT_NAME:-minesweeper_agent}"
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${CKPT_DIR:-./model-ckpts}/${WANDB_PROJECT}/${CKPT_DIR_BASENAME}_BS${TRAIN_BATCH_SIZE}_MPL${MAX_PROMPT_LENGTH}_MRL${MAX_RESPONSE_LENGTH}_MS${MAX_STEPS}-${ORACLE_ADV_MODE}}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-${CKPT_DIR_BASENAME}_BS${TRAIN_BATCH_SIZE}_MPL${MAX_PROMPT_LENGTH}_MRL${MAX_RESPONSE_LENGTH}_MS${MAX_STEPS}-${ORACLE_ADV_MODE}}"
export WANDB_RUN_ID="${WANDB_RUN_ID:-${WANDB_RUN_NAME}}"
export WANDB_RESUME="allow"

TRAINING_CMD=(
python3 -m examples.minesweeper.train_minesweeper_agent
...
"+rllm.env.env_args.oracle_solver_mode=${ORACLE_SOLVER_MODE}"
)
