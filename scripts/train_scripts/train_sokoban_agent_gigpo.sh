...
case "${DATASET}" in
  id)
    DATASET_DIFFICULTY="${DATASET_DIFFICULTY:-id-game_over_1-tp1_bs16_n8-run1}"
...
CKPT_DIR_BASENAME="${CKPT_DIR_BASENAME:-${MODEL_BASENAME}-sokoban-agent-${DATASET_DIFFICULTY}}"
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-${MODEL_BASENAME}-sokoban-agent-${DATASET_DIFFICULTY}}"
export MODEL_NAME="${MODEL_NAME:-${CKPT_DIR_BASENAME}_BS${TRAIN_BATCH_SIZE}_MPL${MAX_PROMPT_LENGTH}_MRL${MAX_RESPONSE_LENGTH}_MS${MAX_STEPS}-${GAME_OVER_SUFFIX}}"

agentrl::setup_log_dirs
agentrl::maybe_enable_xtrace

export TIMESTAMP="${TIMESTAMP:-$(date +%Y%m%d%H%M%S)}"
export WANDB_PROJECT="${WANDB_PROJECT:-rLLM-Sokoban-RL-final}"
# export WANDB_PROJECT="${WANDB_PROJECT:-rLLM-RL-gigpo_debug}"
export WANDB_API_KEY="${WANDB_API_KEY:-}"
export WANDB_MODE="${WANDB_MODE:-online}"
export WANDB_DIR="${WANDB_DIR:-${REPO_ROOT:-.}/outputs/sokoban-final}"
...
TRAINING_CMD=(
python3 -m examples.sokoban.train_sokoban_agent_gigpo
