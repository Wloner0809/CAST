#!/usr/bin/env bash
# Single-environment evaluation script for rllm — CONCURRENT-SAFE variant.
#
# Concurrency-safe copy of ../eval_unified.sh. Two deltas: the source path has
# one extra ../ (this file lives in lib/), and agentrl::ray_cleanup is omitted --
# it runs a host-wide `ray stop --force` that would kill sibling evals. Concurrent
# callers should also set a distinct RAY_EVAL_TEMP_DIR.
#
...
# eval_batch.sh (open-source) or eval_closed.sh (closed-source API).
#
# Usage:
#   bash eval_unified_concurrent.sh <model_path> <config_yaml> [output_dir] [-- extra_cli_args...]
#
...
# Examples:
#   # Single evaluation
#   bash eval_unified_concurrent.sh /path/to/model rllm/eval/config/sokoban.yaml
#
#   # With output dir and overrides
#   EXTRA_OVERRIDES="sampling.temperature=0.6" \
#   bash eval_unified_concurrent.sh /path/to/model rllm/eval/config/sokoban.yaml /path/to/output
#
#   # Pass arbitrary CLI args after --
#   bash eval_unified_concurrent.sh /path/to/model rllm/eval/config/sokoban.yaml /path/to/output -- --max-steps 200
...
# NOTE: agentrl::ray_cleanup (ray stop --force) intentionally OMITTED here — it
# would kill other concurrent evals' Ray clusters. See header. Each concurrent
# caller isolates Ray via a distinct RAY_EVAL_TEMP_DIR instead.
