"""Minesweeper training entrypoint using GiGPO advantage estimation.

This is the GiGPO variant of ``train_minesweeper_agent.py``.  The only
...
Mirrors ``examples/sokoban/train_sokoban_agent_gigpo.py``.
"""
...
def _print_run_info(config, train_dataset, val_dataset):
    ...
    print(sep)
    print("  MINESWEEPER TRAINING RUN INFO (GiGPO)")
    print(sep)
    print(f"  mode       : gigpo")
