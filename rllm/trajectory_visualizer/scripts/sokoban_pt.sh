#!/usr/bin/env bash
#
# Launch the Sokoban trajectory visualizer (.pt variant).
#
# Serves rLLM EVAL outputs serialized as torch .pt files (list[Trajectory]),
# i.e. sokoban_trajectories.pt produced by the closed-model n4 evals. This is
# the counterpart to sokoban.sh, which instead serves training-checkpoint
# chat_completions/*.jsonl. Correctness here is success-based (info['success']).
#
# Data live under:
#   OUT_ROOT/<model>/<run_tag>/sokoban/<difficulty>/<model>/sokoban_trajectories.pt
#
# Usage:
#   ./sokoban_pt.sh                          # default target (first .pt found, see below)
#   ./sokoban_pt.sh <relative-path>          # a path under OUT_ROOT (dir or .pt file)
#   ./sokoban_pt.sh /abs/path/to/x.pt        # an explicit .pt file
#   ./sokoban_pt.sh /abs/path/dir            # an explicit dir containing .pt files
#   ./sokoban_pt.sh <target> --port 8800 --theme light
#   ./sokoban_pt.sh --list                   # list available .pt trajectory files
#
