#!/usr/bin/env bash
#
# Launch the Minesweeper trajectory visualizer (.pt variant).
#
# Serves rLLM EVAL outputs serialized as torch .pt files (list[Trajectory]),
# i.e. minesweeper_trajectories.pt produced by the closed-model n4 evals. This is
# the counterpart to minesweeper.sh, which instead serves training-checkpoint
# chat_completions/*.jsonl. Correctness here is success-based (info['success']).
#
# Data live under:
#   OUT_ROOT/<model>/<run_tag>/minesweeper/<difficulty>/<model>/minesweeper_trajectories.pt
#
# Usage:
#   ./minesweeper_pt.sh                          # default target (first .pt found, see below)
#   ./minesweeper_pt.sh <relative-path>          # a path under OUT_ROOT (dir or .pt file)
#   ./minesweeper_pt.sh /abs/path/to/x.pt        # an explicit .pt file
#   ./minesweeper_pt.sh /abs/path/dir            # an explicit dir containing .pt files
#   ./minesweeper_pt.sh <target> --port 8800 --theme light
#   ./minesweeper_pt.sh --list                   # list available .pt trajectory files
#
