#!/usr/bin/env bash
#
# Launch the Minesweeper trajectory visualizer.
#
...
#
# Usage:
#   ./minesweeper.sh                         # list available experiments
#   ./minesweeper.sh <experiment-name>       # a ckpt dir name under CKPT_ROOT
#   ./minesweeper.sh /abs/path/chat_completions   # an explicit directory
#   ./minesweeper.sh <name> --port 8800 --theme light
#   ./minesweeper.sh --list                  # list available experiments
#
...
VISUALIZER="${VISUAL_DIR}/minesweeper_visual.py"
...
list_experiments() {
  ...
  echo "  ${CKPT_ROOT}"
  echo
  for d in "${CKPT_ROOT}"/*/chat_completions; do
    [ -d "${d}" ] || continue
