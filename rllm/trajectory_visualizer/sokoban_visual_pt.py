#!/usr/bin/env python3
"""
Sokoban Trajectory Visualizer (.pt variant)
===========================================
A web-based viewer for Sokoban RL evaluation trajectories serialized as torch
`.pt` files (rLLM `list[Trajectory]` format, e.g. sokoban_trajectories.pt).

Correctness is success-based: each trajectory is judged correct/incorrect by
`info['success']`, NOT by the (cumulative, possibly negative) env reward.
Reuses the same HTML/JS frontend as sokoban_visual.py (board rendering,
pagination, group-by-prompt, theme), reading `.pt` files instead of
chat_completions `.jsonl`.

Usage:
    python3 sokoban_visual_pt.py --path <file.pt | dir_of_pt_files> [--port 8766]
"""
...
<title>Sokoban Trajectory Visualizer</title>
<style>
...
/* Sokoban grid — block-level so it sits naturally on its own line(s)
...
.sk-wall { background: var(--cell-wall); color: var(--text-muted); }
.sk-empty { background: var(--cell-empty); color: var(--text-muted); }
.sk-target { background: rgba(79, 195, 247, 0.18); color: var(--accent-blue); }
.sk-box { background: rgba(255, 183, 77, 0.30); color: var(--accent-orange); }
.sk-box-on-target { background: rgba(129, 199, 132, 0.40); color: var(--accent-green); }
.sk-player { background: rgba(206, 147, 216, 0.30); color: var(--accent-purple); }
.sk-player-on-target { background: rgba(206, 147, 216, 0.45); color: var(--accent-purple); }
...
<div class="header">
    <div class="header-content">
        <h1>&#128230; Sokoban Trajectory Visualizer</h1>
...
// Extract the action an assistant turn took. The model emits one of
// Up/Down/Left/Right, normally inside a ``` block (e.g. ```Up```) but sometimes
// with single/double backticks; we accept 1-3 backticks (matching the inline
// highlighter) and prefer the LAST occurrence (the final decision), falling
// back to the last bare line that is exactly a direction word.
function extractAction(content) {
    const text = String(content);
    const fenced = /`{1,3}\s*(Up|Down|Left|Right)\s*`{1,3}/gi;
    let m, last = null;
    while ((m = fenced.exec(text)) !== null) last = m[1];
    if (last) return normalizeAction(last);
    // Fallback: a line that is exactly a direction word.
    let lastBare = null;
    for (const line of text.split('\n')) {
        const t = line.trim();
        if (/^(Up|Down|Left|Right)$/i.test(t)) lastBare = t;
    }
...
// Detect contiguous Sokoban board blocks and render them inline inside a single
// continuous content box, so the grid sits naturally within the text flow (text
// above, board, text below) rather than in a separate box. Lone ``` fence lines
// that merely wrap a board are dropped so the result reads continuously.
//
// A board is a run of >=2 consecutive lines made entirely of Sokoban glyphs
// (# _ O * X P S), all the same width, with at least one wall '#' present (so
// prose lines never get mistaken for a board).
function formatContent(content, role) {
    const lines = String(content).split('\n');
    const glyphRe = /^[#_O*XPS]+$/;            // a pure Sokoban board row
    ...
    // Real Sokoban boards are fully wall-bordered: every row starts and ends
    // with '#'. Require that (plus equal width) so prose never triggers a
    // board, AND require at least one non-wall glyph across the first two rows
    // so an all-'#' markdown rule like "###\n###" is rejected.
    function isWallRow(s) { return s.length >= 3 && glyphRe.test(s) && s[0] === '#' && s[s.length - 1] === '#'; }
    function isBoardStart(idx) {
        const a = lines[idx].trim();
        if (!isWallRow(a)) return false;
        const b = idx + 1 < lines.length ? lines[idx + 1].trim() : '';
        if (!isWallRow(b) || b.length !== a.length) return false;
        return /[_O*XPS]/.test(a + b);
    }
    ...
    while (i < lines.length) {
        if (isBoardStart(i)) {
            ...
            flushText();
            const boardLines = [];
            const width = lines[i].trim().length;
            let j = i;
            while (j < lines.length) {
                const t = lines[j].trim();
                if (isWallRow(t) && t.length === width) {
                    boardLines.push(t);
                    j++;
                } else {
                    break;
...
// Highlight standalone action commands (```Up``` / ```Left``` ... or backtick
...
function renderTextBlock(text, role) {
    ...
    escaped = escaped.replace(/`{1,3}\s*(Up|Down|Left|Right)\s*`{1,3}/gi,
        (m, dir) => '<span class="action-block">' + normalizeAction(dir) + '</span>');
    return '<pre class="content-seg">' + escaped + '</pre>';
}

function renderBoard(boardLines) {
...
// Sokoban glyphs: # Wall | _ Empty | O Target | X Box | * Box on target |
// P Player | S Player on target.
function renderCell(sym) {
    if (sym === '#') return '<span class="sk-cell sk-wall">&#9608;</span>';
    if (sym === '_') return '<span class="sk-cell sk-empty"></span>';
    if (sym === 'O') return '<span class="sk-cell sk-target">&#9678;</span>';
    if (sym === 'X') return '<span class="sk-cell sk-box">&#9632;</span>';
    if (sym === '*') return '<span class="sk-cell sk-box-on-target">&#9635;</span>';
    if (sym === 'P') return '<span class="sk-cell sk-player">&#9679;</span>';
    if (sym === 'S') return '<span class="sk-cell sk-player-on-target">&#9673;</span>';
    return `<span class="sk-cell sk-empty">${escapeHtml(sym)}</span>`;
}
...
def main():
    parser = argparse.ArgumentParser(description='Sokoban Trajectory Visualizer (.pt)')
    ...
    print("\U0001F4E6 Sokoban Trajectory Visualizer (.pt)")
    print(f"{'-' * 44}")
