...
# Bypass proxy for localhost to avoid Gradio startup issues
os.environ["no_proxy"] = os.environ.get("no_proxy", "") + ",localhost,127.0.0.1"
os.environ["NO_PROXY"] = os.environ.get("NO_PROXY", "") + ",localhost,127.0.0.1"



def main(trajectory_file: str = "./outputs/results/trajectories.pt", server_port: int = 12345):
    ...
    def get_task_key(trajectory):
        """Get a unique key for the task, excluding rollout_id for aggregation."""
        task = getattr(trajectory, "task", {}) or {}
        # Create a key from task fields, excluding _rollout_id
        key_parts = []
        for k, v in sorted(task.items()):
            if k != "_rollout_id":
                key_parts.append(f"{k}:{v}")
        return "|".join(key_parts) if key_parts else str(id(trajectory))

    def aggregate_trajectories(trajs):
        """Aggregate trajectories by task (for n_rollout per prompt)."""
        from collections import defaultdict
        task_groups = defaultdict(list)
        for traj in trajs:
            key = get_task_key(traj)
            task_groups[key].append(traj)

        aggregated = []
        for key, group in task_groups.items():
            # Create aggregated entry with best trajectory and stats
            rewards = [float(t.reward) for t in group]
            best_idx = rewards.index(max(rewards))
            best_traj = group[best_idx]
            ...
            agg_info = {
                ...
                "all_trajectories": group,
            }
            # Attach aggregation info to best trajectory
            best_traj._aggregation_info = agg_info
            aggregated.append(best_traj)

        return aggregated

    def filter_trajectories_by_reward(filter_option: str, aggregate_mode: bool = False):
        """Filter trajectories based on reward"""
        base_trajs = aggregate_trajectories(all_trajs) if aggregate_mode else all_trajs

        if filter_option == "All Trajectories":
            return base_trajs
        elif filter_option == "Zero Reward (Failed)":
            return [t for t in base_trajs if float(t.reward) == 0.0]
        elif filter_option == "Nonzero Reward (Partial/Full Success)":
            return [t for t in base_trajs if float(t.reward) > 0.0]
        elif filter_option == "Perfect Score (Reward = 1)":
            return [t for t in base_trajs if float(t.reward) == 1.0]
        else:
            return base_trajs

    def format_exploration_trajectory(exploration_traj):
        """Format exploration trajectory for display."""
        if not exploration_traj:
            return "*No exploration trajectory*"

        formatted_parts = []
        for i, step in enumerate(exploration_traj):
    ...
    def update_step_view(traj_idx: int, step_idx: int, filter_option: str, aggregate_mode: bool = False):
        """Update the step view with detailed information - updated for new API pattern"""
        empty_content = "*No data available*"

        filtered_trajs = filter_trajectories_by_reward(filter_option, aggregate_mode)

        if traj_idx >= len(filtered_trajs):
            return (empty_content,) * 13
        ...
        # Get world modeling and exploration trajectory from trajectory info
        traj_info = getattr(trajectory, "info", {}) or {}
        world_modeling_text = traj_info.get("world_modeling", "*No world modeling available*")
        if not world_modeling_text:
            world_modeling_text = "*No world modeling available*"

        exploration_traj = traj_info.get("exploration_trajectory", [])
        if exploration_traj:
            exploration_traj_text = format_exploration_trajectory(exploration_traj)
        else:
            exploration_traj_text = "*No exploration trajectory available*"

        # Get aggregation info if available
        agg_info = getattr(trajectory, "_aggregation_info", None)
        if agg_info:
            aggregation_text = f"**Rollouts:** {agg_info['n_rollouts']}\n"
            aggregation_text += f"**Mean Reward:** {agg_info['mean_reward']:.3f}\n"
            aggregation_text += f"**Max Reward:** {agg_info['max_reward']:.3f}\n"
            aggregation_text += f"**Min Reward:** {agg_info['min_reward']:.3f}\n"
            aggregation_text += f"**Success Rate:** {agg_info['success_rate']*100:.1f}%\n"
            aggregation_text += f"**All Rewards:** {[f'{r:.3f}' for r in agg_info['rewards']]}"
        else:
            aggregation_text = "*Not in aggregate mode or single rollout*"

        if num_steps == 0:
            return (position_text, metadata_text, perf_text, question_text, empty_content, empty_content, empty_content, empty_content, empty_content, empty_content, world_modeling_text, exploration_traj_text, aggregation_text)
        ...
        return (position_text, metadata_text, perf_text, question_text, thinking_text, response_text, step_perf_text, actions_text, outputs_text, final_answer_text, world_modeling_text, exploration_traj_text, aggregation_text)
    ...
    with gr.Blocks(title="Agent Trajectory Visualizer") as interface:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                aggregate_checkbox = gr.Checkbox(label="📊 Aggregate Mode (group by task)", value=False, interactive=True)
            with gr.Column(scale=1):
        ...
        with gr.Row():
            with gr.Column(scale=1):
                with gr.Accordion("🌍 World Modeling", open=False):
                    world_modeling_output = gr.Textbox(label="World Model", lines=10, interactive=False, elem_classes=["metadata-box"])

            with gr.Column(scale=1):
                with gr.Accordion("🗺️ Exploration Trajectory", open=False):
                    exploration_traj_output = gr.Markdown(elem_classes=["outputs-box"])

            with gr.Column(scale=1):
                with gr.Accordion("📈 Aggregation Stats", open=False):
                    aggregation_output = gr.Markdown(elem_classes=["performance-box"])

        all_outputs = [current_pos_display, metadata_output, performance_output, question_output, thinking_output, response_output, step_perf_output, actions_output, outputs_output, final_answer_output, world_modeling_output, exploration_traj_output, aggregation_output]

        def reset_to_first_trajectory():
            return 0, 0

        prev_traj_button.click(fn=lambda t, s, f, a: advance_step_or_trajectory(t, s, "prev", "trajectory", filter_trajectories_by_reward(f, a)), inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=[current_traj_idx_state, current_step_idx_state])
        next_traj_button.click(fn=lambda t, s, f, a: advance_step_or_trajectory(t, s, "next", "trajectory", filter_trajectories_by_reward(f, a)), inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=[current_traj_idx_state, current_step_idx_state])
        prev_step_button.click(fn=lambda t, s, f, a: advance_step_or_trajectory(t, s, "prev", "step", filter_trajectories_by_reward(f, a)), inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=[current_traj_idx_state, current_step_idx_state])
        next_step_button.click(fn=lambda t, s, f, a: advance_step_or_trajectory(t, s, "next", "step", filter_trajectories_by_reward(f, a)), inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=[current_traj_idx_state, current_step_idx_state])

        filter_dropdown.change(fn=reset_to_first_trajectory, outputs=[current_traj_idx_state, current_step_idx_state])
        aggregate_checkbox.change(fn=reset_to_first_trajectory, outputs=[current_traj_idx_state, current_step_idx_state])

        current_traj_idx_state.change(fn=update_step_view, inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=all_outputs)
        current_step_idx_state.change(fn=update_step_view, inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=all_outputs)
        filter_dropdown.change(fn=update_step_view, inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=all_outputs)
        aggregate_checkbox.change(fn=update_step_view, inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=all_outputs)

        interface.load(fn=update_step_view, inputs=[current_traj_idx_state, current_step_idx_state, filter_dropdown, aggregate_checkbox], outputs=all_outputs)

    interface.launch(share=True, server_port=server_port, theme=gr.themes.Soft(), css=custom_css)
