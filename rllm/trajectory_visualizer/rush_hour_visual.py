...
===============================
A web-based viewer for Rush Hour RL rollout/validation trajectories
...
Usage:
    python3 rush_hour_visual.py --path <chat_completions_dir> [--port 8767]

Example:
    python3 rush_hour_visual.py --path .../chat_completions
"""
...
def assemble_json_array(filepath):
    """Build a JSON-array body from a .jsonl file WITHOUT re-parsing.

    Each non-empty line is already a valid JSON object, so we splice the raw
    bytes into ``[obj,obj,...]`` directly. This skips the expensive
    json.loads -> json.dumps round-trip (which doubled CPU and latency on the
    20-40 MB trajectory files) and preserves the exact original content.
    """
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if parsed.path == '/' or parsed.path == '/index.html':
                ...
                # Assemble the JSON array directly from raw lines (no reparse),
                # then gzip it when the client accepts gzip. Trajectory files are
                # large and compress well, so this substantially cuts transfer time.
                body = assemble_json_array(filepath)
...
def main():
    parser = argparse.ArgumentParser(description='Rush Hour Trajectory Visualizer')
    ...
    print("\U0001F697 Rush Hour Trajectory Visualizer")
    print(f"{'-' * 44}")
