...
class VerlProxyManager(ProxyManager):
    ...
    def _instrument_vllm_servers(self) -> None:
        ...
        try:
            ...
            if support == "none":
                logger.warning("vLLM < 0.10.2 detected, but VERL servers are already running. Token IDs will NOT be available! To enable token IDs, call instrument_vllm() BEFORE creating AgentLoopManager.")
            elif support == "native":
