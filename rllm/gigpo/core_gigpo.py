...
def build_step_group(anchor_obs: np.array, index: np.array, enable_similarity: bool = False, similarity_thresh: float = 0.95, summarize: bool = False, return_group_sizes: bool = False):
    """
    ...
    list[int] (only if return_group_sizes=True)
        Size of each distinct step-level group (one entry per group, order arbitrary).
        Useful for logging group-size distribution metrics (e.g. singleton fraction).
    """
    ...
    if return_group_sizes:
        return step_group_uids, group_size
