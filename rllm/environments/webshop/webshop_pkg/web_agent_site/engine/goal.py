...
def _load_spacy_model(model_name: str = "en_core_web_sm"):
    """Load spacy model from shared path or fallback to default.

    Priority:
    1. SPACY_MODEL_DIR env var (explicit override)
    2. Shared path under the repo data dir (avoids re-downloading per job)
    3. Default spacy.load (requires `python -m spacy download`)
    """
    # 1) Explicit env override
    env_dir = os.environ.get("SPACY_MODEL_DIR")
    if env_dir:
        model_path = os.path.join(env_dir, model_name)
        if os.path.isdir(model_path):
            return spacy.load(model_path)

    # 2) Shared path under the repo data dir
    shared_path = os.path.join(
        os.path.dirname(__file__),  # .../web_agent_site/engine/
        "..", "..", "..", "..",      # -> rllm repo root (approximate)
        "data", "spacy_models", model_name,
    )
    shared_path = os.path.normpath(shared_path)
    if os.path.isdir(shared_path):
        return spacy.load(shared_path)

    # 3) Default: installed via `python -m spacy download`
