...
# Set Java 11 for pyserini (requires Java 11+). RPM installs carry a
# build-specific suffix, so glob first and fall back to the generic names.
for _jdk in /usr/lib/jvm/java-11-openjdk* /usr/lib/jvm/java-11-openjdk-amd64 /usr/lib/jvm/java-11; do
  if [ -d "$_jdk" ]; then
