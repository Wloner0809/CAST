...
# Navigation tips aligned with RAGEN's RENDER_INSTRUCTIONS
RENDER_INSTRUCTIONS = (
    "Here are some useful tips to help you navigate through the website more effectively and efficiently:\n"
    "1. We must buy a product within 10 actions. It doesn't have to match perfectly with description.\n"
    "2. Search term should not include details like size, color.\n"
    "3. Never search for more than 2 times.\n"
    "4. Do not be too strict about the description, it's more important to buy one that is close enough within action limit.\n"
    "5. Prioritize click a product in the current page over going to next page.\n"
    "6. Almost never click[next >] for more than 2 times.\n"
    "7. Almost never click[< prev] unless you are sure the product is on one of the previous pages.\n"
    "8. If you have less than 3 actions left, just buy the first product you see in the current page.\n"
    "9. If a matching option exists, make sure to click[size] then click[color], one at a time, before click[buy now], "
    "but don't have to if only 1 action left, in that case you just click[buy now]. Never click description."
)
...
class WebShopWorker:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # Ensure Java 11 is available for pyserini/anserini (requires class file version 55.0).
        # Without this, Ray worker subprocesses may inherit Java 8 from the system default,
        # causing UnsupportedClassVersionError when jnius loads anserini classes.
        # A preset JAVA_HOME is honoured only when it really is 11+ (a system
        # default pointing at Java 8 must not win); otherwise probe the usual
        # install locations (RPM installs carry a build-specific suffix).
        def _is_java11_plus(home: str) -> bool:
            if not home or not os.path.isdir(os.path.join(home, "lib", "server")):
                return False
            ...
            try:
                ...
                pass
            # No parsable release file: fall back to the directory name.
            return "java-11" in home or "java-17" in home or "java-21" in home

        _java11_home = os.environ.get("JAVA_HOME") or ""
        if not _is_java11_plus(_java11_home):
            _candidates = sorted(glob.glob("/usr/lib/jvm/java-11-openjdk*")) + [
                "/usr/lib/jvm/java-11-openjdk-amd64",
                "/usr/lib/jvm/java-11",
                "/usr/lib/jvm/default-java",
            ]
            _java11_home = next((p for p in _candidates if _is_java11_plus(p)), "")
        if _java11_home and os.environ.get("JAVA_HOME") != _java11_home:
            os.environ["JAVA_HOME"] = _java11_home
            os.environ["JVM_PATH"] = os.path.join(_java11_home, "lib", "server", "libjvm.so")
            os.environ["PATH"] = os.path.join(_java11_home, "bin") + ":" + os.environ.get("PATH", "")

        import gym

        # Add webshop_pkg to path
        ...
        self._env = WebAgentTextEnv(**env_kwargs)
    ...
    def _rebuild_env(self, seed: int | None = None) -> None:
        ...
        # Re-imported here because __init__'s import is local to that function;
        # sys.path already carries webshop_pkg, and the module is cached.
        from web_agent_site.envs import WebAgentTextEnv
    ...
    def step(self, action: str) -> tuple[str, float, bool, dict]:
        ...
        info = {
            ...
            "clickables": self._available_actions.get("clickables", []),
            "task_score": reward,
            "observation_changed": obs != previous_obs,
        }
    ...
    def get_observation(self) -> str:
        """Get the current observation text."""
...
class WebShopWorkerPoolActor:
    ...
    async def acquire_worker(
        ...
        with self._lock:
            ...
            # Capture the running event loop so sync signalers (release_worker /
            # shutdown, which run in Ray's threadpool — NOT on this loop) can wake
            # this coroutine via call_soon_threadsafe. asyncio.Event is not
            # thread-safe; calling event.set() directly from another thread does
            # not reliably wake `await event.wait()`.
        ...
        try:
            ...
            # Race-safe cleanup. release_worker pops the request and assigns a
            # worker under self._lock, so take the same lock here to serialize
            # against it. Two outcomes are possible:
            #   1. We remove our request before release fulfills it -> clean timeout.
            #   2. release already popped us and set result_holder["worker"] in the
            #      race window -> we must NOT leak that worker. Return it to the
            #      caller instead of failing, since it was assigned to this request.
            with self._lock:
                self._pending_requests[pool_key] = [
                    r for r in self._pending_requests[pool_key]
                    if r["request_id"] != request_id
                ]
                assigned_worker = result_holder["worker"]

            if assigned_worker is not None:
                logger.debug(
                    f"Request {request_id[:8]} timed out but a worker was assigned "
                    f"in the race window; using it instead of failing."
                )
    ...
    def release_worker(
        ...
        with self._lock:
            ...
            if self._pending_requests[pool_key]:
                ...
                self._signal_request(request_info)
    ...
    @staticmethod
    def _signal_request(request_info: dict):
        """Wake a queued acquire_worker coroutine in a thread-safe way.

        release_worker / shutdown run in Ray's threadpool (off the actor's event
        loop), so they must schedule event.set() onto the loop that owns the
        Event rather than calling it directly (asyncio.Event is not thread-safe).
        """
        loop = request_info.get("loop")
        event = request_info["event"]
        if loop is not None and not loop.is_closed():
            loop.call_soon_threadsafe(event.set)
        else:
            # Fallback: no captured loop (shouldn't happen for queued requests).
            event.set()
    ...
    def shutdown(self):
        ...
        for pool_key, requests in self._pending_requests.items():
            for request_info in requests:
                self._signal_request(request_info)  # Unblock waiters (they'll get None)
            self._pending_requests[pool_key] = []
...
class WebShopEnv(BaseEnv):
    ...
    def step(self, action: str) -> tuple[str, float, bool, dict]:
        ...
        Returns:
            A tuple of (observation, reward, done, info) where:
                ...
                - reward: 1.0 if purchase successful with perfect match, 0.0 otherwise
                - done: Whether the episode has ended
                - info: Dictionary with available_actions, task_score, success, etc.
        """
        self._lazy_init()

        self._step_count += 1
        previous_obs = self._observation
        # Validity must be judged against the page the action was issued on.
        # self._available_actions is overwritten with the post-step page below,
        # so snapshot the pre-step actions before calling the worker.
        previous_available_actions = self._available_actions

        # Execute action via Ray worker with actor death handling
        try:
            obs, raw_reward, done, worker_info = ray.get(self._worker.step.remote(action))
        except ray.exceptions.ActorDiedError as e:
            logger.error(f"Worker died during step: {e}")
            # The actor is dead and unrecoverable mid-episode. Drop the handle so
            # the later close() does NOT release a dead worker back into the pool
            # (which would hand a dead actor to the next acquirer).
            self._worker = None
            self._initialized = False
        ...
        # Process reward: 1.0 for perfect purchase, 0.0 otherwise (aligned with RAGEN)
        task_score = worker_info.get("task_score", raw_reward)
        ...
        # Determine action validity against the pre-step page the action was
        # issued on (not the post-step page now in self._available_actions).
        action_is_valid = self._is_valid_action(action, previous_available_actions)

        # Determine if the action was effective (observation changed)
        observation_changed = worker_info.get("observation_changed", obs != previous_obs)
        action_is_effective = observation_changed

        # Determine if we're at the end of available pages
        formatted_actions = self.get_available_actions_formatted()
        end_of_page = (
            tuple(formatted_actions) == ("click[back to search]", "click[< prev]")
        )

        # Detect purchase attempt
        action_lower = action.lower().strip()
        success_purchase = done and "click[buy now]" in action_lower
    ...
    def _is_valid_action(self, action: str, available_actions: dict | None = None) -> bool:
        """Check if an action is valid against a given page's available actions.

        Args:
            action: The action string (e.g. "search[query]" or "click[element]").
            available_actions: The available-actions dict to validate against.
                Defaults to the current page (self._available_actions) when not
                given. During step() the caller passes the pre-step snapshot so
                validity reflects the page the action was actually issued on.
        """
        ...
        if action_lower.startswith("search["):
            return available_actions.get("has_search_bar", False)
    ...
    def get_available_actions(self) -> dict:
        """Get available actions for the current state (raw dict from underlying env)."""
        return self._available_actions

    def get_available_actions_formatted(self) -> list[str]:
        """Get available actions as a formatted list of strings.

        Returns a list like: ["search[<content>]", "click[item1]", "click[buy now]", ...]
        Filters out 'search' from clickables and removes 'next >' at end of page.
        """
        formatted = []

        if self._available_actions.get("has_search_bar", False):
        ...
        # Remove 'click[next >]' when at end of page (only back/prev remain)
    ...
    def success(self) -> bool:
        """Whether the current episode was completed successfully."""
        return self._won

    def render(self, mode: str = "text") -> str:
        """Render the current state with navigation tips and available actions.

        Returns the current observation combined with RENDER_INSTRUCTIONS
        and the formatted list of available actions.
        """
        if not self._observation:
            return ""

        available_actions = self.get_available_actions_formatted()
        rendered = self._observation + ".\n"
        rendered += RENDER_INSTRUCTIONS + "\n"
        rendered += "You must choose from these actions: " + ", ".join(available_actions) + "."
        return rendered
