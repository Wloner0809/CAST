...
class ModelOutput:
    ...
    logprobs: list[float] | None = None  # completion logprobs (logprob of selected token)
    prompt_logprobs: list[float] | None = None  # prompt logprobs aligned to prompt_ids
    # token_entropies: per-token entropy computed from full logits distribution (if available)
    token_entropies: list[float] | None = None
    prompt_length: int = 0
    completion_length: int = 0
    finish_reason: str | None = None
    stop_token_id: int | None = None  # The actual stop token generated (e.g., <|im_end|> or <|endoftext|>)
