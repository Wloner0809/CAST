...
class VerlEngine(RolloutEngine):
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        # Always remove `validate` from kwargs before forwarding sampling params.
        # Otherwise, when self.validate is True, short-circuiting can leave
        # `validate` in kwargs and accidentally pass it into backend sampling kwargs.
        validate_flag = kwargs.pop("validate", False)
        validate = self.validate or validate_flag
        ...
        if hasattr(self.chat_parser, "stop_token_ids") and "stop_token_ids" not in sampling_params:
            sampling_params["stop_token_ids"] = self.chat_parser.stop_token_ids
        ...
        # Detect which stop token the model actually generated.
        # Qwen2.5 may generate <|endoftext|> instead of <|im_end|>.
        # parse_assistant must use the matching eot string to avoid assemble_steps mismatch.
        actual_stop_token_id = None
        if completion_ids and hasattr(self.chat_parser, 'stop_token_ids'):
            if completion_ids[-1] in self.chat_parser.stop_token_ids:
                actual_stop_token_id = completion_ids[-1]
        ...
        return ModelOutput(
            ...
            stop_token_id=actual_stop_token_id,
        )
