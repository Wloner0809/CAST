...
class DatasetRegistry:
    ...
    # Dataset directory at project root (rllm/data/datasets -> data/datasets)
    _PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
    _DATASET_DIR = os.path.join(_PROJECT_ROOT, "data", "datasets")
    ...
    def _resolve_path(cls, path: str) -> str:
        """Expand a registry entry into an absolute path.

        Registry entries are stored relative to the project root so the file
        stays portable across checkouts; absolute entries are passed through
        unchanged for datasets kept outside the repo.
        """
    ...
    @classmethod
    def _relativize_path(cls, path: str) -> str:
        """Store a path relative to the project root when it lives inside it."""
        abs_path = os.path.abspath(path)
        if abs_path == cls._PROJECT_ROOT or abs_path.startswith(cls._PROJECT_ROOT + os.sep):
            return os.path.relpath(abs_path, cls._PROJECT_ROOT)
        return abs_path
    ...
    def _load_registry(cls) -> dict[str, dict[str, str]]:
        ...
        return {name: {split: cls._resolve_path(path) for split, path in splits.items()} for name, splits in registry.items()}

    @classmethod
    def _save_registry(cls, registry: dict[str, dict[str, str]]) -> None:
        """Save the dataset registry to the registry file."""
        cls._ensure_directories()
        registry = {name: {split: cls._relativize_path(path) for split, path in splits.items()} for name, splits in registry.items()}
